...
class GetBaiduRank(threading.Thread):
	...
	def __init__(self, keyWord, fileline):
	...
	def run(self):
		...
		for i in range(10,51,10):
			print i
			para = urllib.urlencode({'pn':i,'word':self.keyWord,'sa':'np','st':'111081'})
		...
		for web in self.local.urlRank:
			all50.write(str(self.fileline)+"\t"+str(self.local.urlRank.index(web))+"\t"+web+"\n")
			try:
				sheet.write(self.fileline,self.local.urlRank.index(web) ,web)
...
def main():
	...
	for keyWord in sourceFile:
		...
		print keyWord
		k = GetBaiduRank(keyWord,fileline)
		k.setDaemon(True)
		k.start()
		k.join()
